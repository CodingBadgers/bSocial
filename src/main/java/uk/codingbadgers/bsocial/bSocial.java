...
public class bSocial extends Plugin {
	...
	@Getter private static bSocial instance;
	@Getter private static Config config;
	@Getter private static ChatterManager chatterManager;
	@Getter private static ChannelManager channelManager;
	...
	private void loadConfig() throws ConfigException {
		...
		try{
			...
			if (config == null || config.getConfigVersion() != CURRENT_CONFIG_VERSION) {
				...
				getLogger().warning("The old config will be located at \"plugins/bSocial/" + backup.getName() + "\".");
